...
\newpage
\hsec{Part II: pivoting away from interpolation --- the 180-degree rotation task}

Section~9's four candidate fixes all assume the underlying interpolation idea is sound and
just needs a smoother latent space, a denser training signal, or a more forgiving target. The
next work session took a different approach entirely: rather than continuing to chase the
$A\!\to\!B$ interpolation problem, pivot to a self-supervised task with the one property the
interpolation task fundamentally lacked --- \emph{exact ground truth} for every prediction,
so progress could finally be measured against a known-correct answer instead of only judged
by eye.

\hsub{10. The new task: learn to rotate a phantom 180 degrees}
Every phantom in the dataset is augmented with $8\times$ in-plane rotation at $45^\circ$
increments (orientation $o \in \{0,\dots,7\}$, Section~1). Because $180^\circ = 4\times45^\circ$,
orientation $o$ and orientation $(o+4)\bmod 8$ of the \emph{same} underlying phantom are an
exact, bit-for-bit $180^\circ$ pair --- confirmed lossless, since \texttt{torchvision}'s
nearest-neighbor rotation at exactly $180^\circ$ reduces to a pure array flip with no
interpolation artifacts. This gives a new task with a property interpolation never had: pick
any phantom at any orientation as $x_0$, and its exact target $x_1$ is known in closed form,
with no scan, no heuristic, and no synthetic blending needed.
\begin{equation}
x_0 = \text{rotate}(x_{\text{raw}}, o), \qquad x_1 = \text{rotate}(x_{\text{raw}}, (o+4)\bmod 8).
\label{eq:rotate180-pair}
\end{equation}
Splitting phantoms by whether the lump sits right or left of the volume's vertical center
line ($w \gtrless 63.5$, ``set A'' vs ``set B'') confirmed the framing geometrically: across
all 984 augmented instances, the split came out exactly balanced (492/492) with zero
ambiguous cases, and by construction every $(o, o{+}4)$ pair lands on opposite sides --- so
training bidirectionally on all 8 orientations as $x_0$ automatically covers both
$A\!\to\!B$ and $B\!\to\!A$ without any explicit filtering.

Because $x_0$ is now always real data (never Gaussian noise), generation no longer needs the
inversion step at all: sampling is a single forward integration of
Eq.~\eqref{eq:euler-fwd} starting from the real $x_0$. This sidesteps the entire
interpolation failure mode from Part I by construction --- there is no noise-space midpoint
to be inaccurate about.

\hsub{11. First result: loss goes down, nothing moves}
Training with the unmodified loss from Eq.~\eqref{eq:loss} (now with $x_0$ real instead of
noise) produced a striking failure: \texttt{train/val\_loss} dropped substantially over
hundreds of epochs, yet the \emph{decoded} prediction was --- voxel for voxel, checked
programmatically, not just by eye --- \textbf{100\% identical to $x_0$} on every one of 10
tested validation pairs, unaffected by using up to $20\times$ more ODE integration steps at
inference. The resolution came from separating two different questions: is the raw,
continuous velocity prediction directionally correct (measured \emph{on} the exact training
line, i.e. with an oracle view of $x_1$), versus does the actual multi-step, self-directed
generation process reach the target. A broader check (20 validation pairs, several $t$ values,
$t=0.5$ excluded as the one point where the one-hot blend is maximally ambiguous regardless of
training quality) showed the network already had a strong, consistent \emph{directional}
signal ($\cos\text{-sim}\approx 0.65$) at that point in training --- the problem was pure
\emph{magnitude}: predicted velocity was only $\sim$20\% of the size needed to actually flip a
voxel's class after integration, a classic symptom of plain MSE regression hedging toward an
undercommitted answer once loss is already fairly low.

\hsub{12. Fix 1: a cross-entropy term on the endpoint estimate}
Since $v^* = x_1-x_0$ is constant along the line, algebra gives an ``endpoint estimate'' from
any point on the trajectory using only quantities already computed in the forward pass:
\begin{equation}
\hat x_1 \;=\; x_t + (1-t)\,v_\theta(x_t,t), \label{eq:endpoint-estimate}
\end{equation}
exact when $v_\theta = v^*$. A first attempt supervised this with an \emph{MSE} loss against
the true $x_1$ --- but algebraically, $(\hat x_1 - x_1)^2 = (1-t)^2(v_\theta - v^*)^2$: exactly
$(1-t)^2$ times the existing velocity loss, same minimizer, same gradient direction, not a new
training signal at all (caught before wasting a training run on it). The actual fix uses
\textbf{cross-entropy} instead:
\begin{equation}
...
\label{eq:pixel-loss}
\end{equation}
Unlike MSE, cross-entropy's gradient does \emph{not} shrink as a prediction approaches
``good enough'' -- it stays large as long as the wrong class is still winning at a voxel,
directly countering the undercommitment found in Section~11. This worked: the decoded
prediction started moving for the first time, with real, substantial repositioning (roughly a
50--60\% reduction in lump-centroid distance versus doing nothing) within the first $\sim$150
epochs of a fresh 1000-epoch run.

\hsub{13. The centroid metric lies: a new failure mode}
Direct visual inspection of the trained predictions (not just their summary statistics) told
a different story than the centroid-distance numbers: predictions were a \textbf{massive,
fragmented crescent-shaped blob} covering much of the volume's cross-section, not a small
relocated lump --- confirmed quantitatively as \textbf{$\sim$24 disconnected predicted lump
components versus $\sim$1 expected} on real phantoms. Centroid distance and overall
voxel-agreement both stayed misleadingly reasonable throughout, because a large blob's center
of mass can coincidentally land near the true position, and because background voxels
dominate voxel-agreement regardless. The root cause is a specific property of PyTorch's
class-weighted cross-entropy: the \texttt{weight} argument scales the loss by each voxel's
\emph{true} class, not its predicted one, so missing a real lump voxel costs up to
$12.57\times$ but wrongly predicting lump at a true-background voxel costs only $1.0\times$
--- a direct, asymmetric incentive to over-predict the rare classes wherever the network is
unsure.

\hsub{14. Fix 2: symmetric volume regularization, a decaying schedule, and EMA}
Three complementary changes addressed this:
\begin{itemize}[nosep]
\item \textbf{Soft Dice loss} on the lump/pillar channels, added alongside
  Eq.~\eqref{eq:pixel-loss}:
  \begin{equation}
  \mathcal{L}_{\text{dice}} = 1 - \frac{2\sum_v p_v t_v + 1}{\sum_v p_v + \sum_v t_v + 1},
  \qquad \mathcal{L} \mathrel{+}= \lambda_{\text{dice}}\,\mathcal{L}_{\text{dice}}.
  \label{eq:dice}
  \end{equation}
  Unlike cross-entropy, Dice is symmetric in false positives and false negatives --- predicting
  excess volume directly grows the denominator without growing the overlap numerator, so
  over-prediction is penalized even where CE does not penalize it.
\item \textbf{Decaying $\lambda_{\text{pixel}}$}: linearly interpolated from $1.0$ down to
  $0.3$ over training (by absolute epoch, so it continues correctly across a resumed run) ---
  cross-entropy's magnitude push has no natural ceiling and was found to sustain a
  $\sim$1.25--1.3$\times$ magnitude overshoot for hundreds of epochs when held constant; fading
  it out lets the bounded, well-behaved velocity MSE and Dice terms dominate refinement later.
\item \textbf{EMA of the model weights} (decay $0.999$, $\sim$10-epoch smoothing window at
  this task's step rate), used for every preview, diagnostic, and checkpoint save instead of
  the raw training weights --- motivated directly by the aggregate lump-centroid-error metric
  swinging between $\sim$17 and $\sim$60 voxels from one 25-epoch checkpoint to the next with
  no clear trend, a classic symptom of noisy per-step weights in diffusion/flow-matching
  training.
\end{itemize}
This combination fixed the blob: predicted lump component counts fell from the 20--60 range
to mostly 2--14, much closer to the $\sim$1 expected.

\hsub{15. A properly trusted metric: mask overlap, not centroid distance}
Even after fix~14, a closer look showed the improvement was still partly illusory. Directly
computing lump-mask \textbf{IoU} and \textbf{Dice} (intersection-over-union and Dice
coefficient between the predicted and true \emph{binary} lump mask, not just their centroids)
gave a mean IoU of only $\approx 0.07$ and Dice $\approx 0.13$ across ten validation pairs ---
versus an exact $0.000$ for a trivial ``never move anything'' baseline (meaningful, since the
two positions are always $180^\circ$ apart and can never overlap by chance), but far from
good. Table~\ref{tab:iou-example} makes the point concretely: a $5\times5$ predicted blob
containing one true voxel scores IoU $= 1/25 = 0.04$ while its centroid can be dead-on. Mask
IoU/Dice (per class and combined lump+pillar) is now the trusted metric throughout the
codebase; centroid distance is kept only as a secondary, easily-misled signal.
\begin{table}[h]
\centering
\begin{tabular}{@{}lccc@{}}
\toprule
Scenario & Centroid error & IoU & Dice \\
\midrule
Never move (trivial baseline) & large & 0.00 & 0.00 \\
Small blob near true position, poor shape & can look small & 0.04--0.10 & 0.08--0.18 \\
Correct, precisely-shaped prediction & $\approx 0$ & $\to 1.0$ & $\to 1.0$ \\
\bottomrule
\end{tabular}
\caption{Why mask overlap and centroid distance can disagree sharply on the same prediction.}
\label{tab:iou-example}
\end{table}

\hsub{16. Ruling out numerical error: it's exposure bias, not integration accuracy}
Before touching the loss again, the live checkpoint was tested for a simpler possible cause:
insufficient integration accuracy. Switching the sampler from Euler to Heun's method
(second-order predictor--corrector, already implemented) and separately tripling the step
count ($30\!\to\!100$) both left mean combined IoU essentially unchanged
($0.094 \to 0.094 \to 0.090$). This rules out numerical discretization error and points
squarely at \emph{exposure bias}: the network was only ever trained at points exactly on the
straight line between a real $(x_0,x_1)$ pair (Eq.~\eqref{eq:path}), so it can fit that
narrow, teacher-forced objective well --- correct direction, roughly correct magnitude ---
while still being inaccurate the moment its own multi-step, self-directed trajectory departs
from that line, which it does almost immediately (measured drift from the true line grows
roughly linearly step over step, reaching a magnitude comparable to the pair's own
$x_0$-to-$x_1$ distance well before the trajectory completes).

\hsub{17. Fix 3: training on the model's own rollout, not just the teacher-forced line}
\textbf{What ``rollout loss'' means, in plain terms.} Every loss used up to this point
(velocity MSE, the cross-entropy/Dice endpoint terms) is computed at a state the network never
had to reach on its own: a point $x_t$ constructed directly from the real, known pair
$(x_0, x_1)$ via Eq.~\eqref{eq:path}. During training the network is only ever asked ``given
this exact point on the true straight line, what's the right velocity?'' --- a question with
an always-correct answer sitting right there in the data. At inference, though, nobody hands
the network a point on the true line after the first step: it has to integrate its own
predicted velocity forward, and any small error in step 1 moves it slightly off the ideal
path, so at step 2 it's answering a question (``what's the velocity \emph{here}?'') for a state
it never trained on. Those small errors compound step after step (Section~16's exposure-bias
finding). \emph{Rollout loss} fixes the mismatch by training on exactly the states the network
itself produces: instead of jumping straight to a known point on the line, let the network take
a few real self-directed steps from $x_0$ --- using its own output as the input to the next
step, exactly as it will at inference --- and only then apply the loss, at wherever the network
actually ended up. This means the network is now practicing on its own mistakes during
training, not just on a clean, idealized version of the task it will never actually see again
once it starts generating on its own.

Formally, a short, gradient-tracked rollout from $x_0$ using the model's own predictions at
each step,
\begin{equation}
x^{(0)} = x_0, \qquad x^{(i+1)} = x^{(i)} + v_\theta\big(x^{(i)}, i\,\Delta t\big)\,\Delta t,
\quad i = 0,\dots,k-1, \quad \Delta t = t_{\text{end}}/k, \label{eq:rollout}
\end{equation}
followed by the same endpoint-estimate trick (Eq.~\eqref{eq:endpoint-estimate}) and the same
cross-entropy + Dice terms (Eqs.~\ref{eq:pixel-loss}, \ref{eq:dice}) evaluated at the rolled-out
state instead of a point on the true line, directly penalizes drift instead of only ever
rewarding accuracy on a path the network will never exactly follow at inference. Unlike
Eq.~\eqref{eq:path}, every step of Eq.~\eqref{eq:rollout} feeds forward through the network with
gradients retained, so this costs $k$ forward/backward passes per training step instead of one
and needs the full computation graph kept live across all $k$ steps.

This loss proved unstable when applied to a freshly-initialized network: with $k{=}3$,
$t_{\text{end}}{=}0.3$, applied every other batch, predicted lump components on a from-scratch
run \emph{grew} across the first three epochs ($754 \to 1794 \to 13370$) rather than settling
down --- plausibly because compounding a still-near-random field across $k$ real self-steps
early in training produces wildly out-of-distribution states, and the resulting large,
noisy gradients destabilize the network before it has any baseline competence to build on.
Applying the identical loss as a \emph{continuation} of an already-trained checkpoint (resumed
from epoch 469 of the fix-14 run) showed no such instability --- component counts stayed low
and stable across five additional epochs. An extended run applying this as a fine-tuning phase
on top of the fix-14 checkpoint was launched to test whether mask IoU/Dice actually trends
upward over a longer horizon; results were not yet available at the time of writing.

\hsub{18. Supporting infrastructure built alongside the modeling work}
\begin{itemize}[nosep]
\item \texttt{RotationPairDataset}: bidirectional $(x_0,x_1)$ pairing via
  Eq.~\eqref{eq:rotate180-pair} for both train and validation splits (orientation is the task
  variable here, not an augmentation nuisance to be dropped from validation).
\item Checkpoint resume support (\texttt{--resume\_from}, plus reattaching to the same wandb
  run id) added specifically so a promising run could be extended rather than restarted from
  scratch --- used repeatedly once the rollout-loss fine-tuning strategy above was adopted.
\item DataLoader \texttt{num\_workers} support: the paired dataset's \texttt{\_\_getitem\_\_}
  does two rotations per item (versus the unconditional task's free noise draw), which was
  starving the GPU at \texttt{num\_workers=0}; parallelizing gave a measured $3.4\times$
  speedup (127s/epoch~$\to$~37s/epoch) with identical loss values, confirming it was a pure
  I/O fix.
\item A 50-pair aggregate diagnostic pass, run periodically during training and logged as a
  proper trend graph, replacing an earlier design that logged one scalar per individual
  preview pair (too noisy pair-to-pair to read a trend off).
\item \texttt{eval\_rotate180.py}: compares any set of checkpoints (e.g. the 100/250/500/1000
  epoch milestones) on the full held-out validation set with exact ground truth, auto-detecting
  each checkpoint's architecture from its own saved config rather than requiring hand-matched
  CLI flags.
\end{itemize}

\hsec{Summary of concrete changes made this session (Part II)}
\begin{itemize}[nosep]
\item Pivoted from noise-space interpolation to a self-supervised $180^\circ$-rotation task
  with exact, closed-form ground truth for every prediction (Section~10).
\item Built and verified the set A/B lump-position split (492/492 balanced, 0 ambiguous) and a
  showcase figure.
\item Diagnosed, by checking the decoded output directly rather than trusting the loss curve,
  that plain velocity MSE never moved the prediction at all; traced this to an undercommitted
  velocity magnitude, not a directional error (Section~11).
\item Added and then \emph{rejected} a redundant MSE-based pixel loss after catching, by
  algebra, that it was an exact reweighting of the existing velocity loss; replaced it with a
  genuinely new cross-entropy signal (Section~12).
\item Found, via direct visual inspection, that the cross-entropy fix produced a massive
  fragmented blob ($\sim$24 components vs $\sim$1 expected) that the centroid/voxel-agreement
  metrics had been hiding; traced this to cross-entropy's asymmetric true-class-only weighting
  (Section~13).
\item Added Dice loss, a decaying pixel-loss schedule, and EMA weights to fix the blob and
  stabilize training (Section~14).
\item Added mask IoU/Dice (lump, pillar, combined) as the trusted evaluation metric,
  demonstrating concretely why centroid distance is not sufficient on its own (Section~15).
\item Ruled out numerical integration error as the remaining bottleneck (Heun's method and
  $3\times$ more steps: no change) and confirmed exposure bias as the root cause (Section~16).
\item Implemented a gradient-tracked rollout-consistency loss that trains directly on the
  model's own multi-step behavior; found it unstable from a fresh initialization but stable as
  a fine-tuning phase on an already-trained checkpoint, and launched an extended run to
  evaluate it (Section~17).
\item Built checkpoint-resume infrastructure, a $3.4\times$ DataLoader speedup, an aggregate
  diagnostic pass, and a cross-checkpoint evaluation script (Section~18).
\end{itemize}

\hsec{Part III --- Cleanup, a rendering bug, and the rollout-loss follow-through}

\hsub{19. Removing the legacy metrics everywhere, not just from the headline number}
Mask IoU/Dice had been adopted as the trusted metric (Section~15), but the older
voxel-agreement and centroid-error numbers were still being computed and logged alongside it
in the training-time diagnostic pass and in wandb, which made the dashboards noisy and easy to
misread at a glance. \texttt{metrics.rotation\_metrics} keeps those legacy fields internally
(cheap to compute, occasionally useful for debugging a specific pair by hand) but every logging
site was audited and cut down to only the six trusted numbers: \texttt{lump\_iou},
\texttt{lump\_dice}, \texttt{pillar\_iou}, \texttt{pillar\_dice}, \texttt{combined\_iou},
\texttt{combined\_dice}. This touched the training loop's diagnostic pass, the qualitative
figure's annotation text box, and both evaluation scripts.

\hsub{20. A machine-wide GPU memory leak in the rendering path}
Shortly after Section~19's cleanup, the qualitative wandb figures went black across every
run. The obvious suspect --- two training processes sharing GPU memory --- turned out to be
wrong: \texttt{nvidia-smi} showed one training process holding 23.8\,GB as a \emph{graphics}
context allocation on a GPU that process never used for its actual PyTorch tensors, i.e. the
leak was in off-screen rendering, not in PyTorch. The renderer (\texttt{PyVista}, wrapping
VTK) never closed its \texttt{Plotter} objects on the off-screen path, so every rendered
figure permanently pinned a VTK graphics context until the GPU's memory was exhausted and
subsequent renders silently came back black. Confirmed via \texttt{git stash}/\texttt{git
stash pop} that this bug pre-existed this session's own changes (it was a latent bug, not a
regression introduced by the metrics cleanup), reproduced it in isolation, then fixed it with
an explicit \texttt{plotter.close()} in the renderer's \texttt{finally} block. Verified by
killing both affected processes, confirming the GPU's memory dropped from 23.8\,GB to
288\,MB, and re-rendering a real phantom to confirm correct (non-black) output.

\hsub{21. Extending and repeating the rollout-loss run}
Section~17 left an open question --- does the rollout-consistency fine-tune actually trend
upward over a longer horizon? Three further training decisions followed directly from
watching that run and wanting a clean answer:
\begin{itemize}[nosep]
\item The fine-tune was restarted under a \emph{fresh} wandb run id rather than continuing to
  log into the pre-cleanup run, so the mask-only dashboard (Section~19) and the rendering fix
  (Section~20) would both be visible from the start of a clean run rather than appearing
  mid-way through an existing one's history.
\item A \emph{second}, fully independent run was launched from scratch using only the
  rollout-consistency loss (no fix-14 warm start), to separate two possible explanations for
  the fine-tune's behavior: is rollout training itself weak, or is it constrained by starting
  from a checkpoint already biased by the earlier losses?
\item After the fine-tune run showed slow but real improvement, it was extended by a further
  500 epochs under the same wandb run id (rather than starting yet another run), while the
  from-scratch run continued independently on a separate GPU. Both were run concurrently once
  GPU headroom allowed it, deliberately waiting for a GPU to free up naturally rather than
  reducing batch size to force a fit on a contended GPU.
\end{itemize}

\hsub{22. IoU versus Dice: why report both}
Both are set-overlap scores between the predicted and true binary mask, $A$ = predicted
voxels, $B$ = true voxels:
\[
\mathrm{IoU} = \frac{|A \cap B|}{|A \cup B|}, \qquad
\mathrm{Dice} = \frac{2|A \cap B|}{|A| + |B|}.
\]
They are monotonic transforms of each other for a \emph{single} pair
($\mathrm{Dice} = 2\,\mathrm{IoU}/(1+\mathrm{IoU})$, so Dice $\geq$ IoU always, with the gap
largest at moderate overlap and vanishing at IoU $\to 0$ or $1$), but that exact relationship
does not carry over to \emph{means} across many pairs, since the transform is nonlinear ---
averaging first and transforming, versus transforming first and averaging, give different
numbers. Both are reported per-class (lump, pillar) and combined so a reader can sanity-check
one against the other rather than trusting a single scalar.

\hsub{23. \texttt{eval\_trajectory.py}: full-dataset metrics plus a trajectory figure}
A new script combines two things the existing \texttt{eval\_rotate180.py} did not do together:
a full-validation-set (and optionally a matched train-sample) metrics pass on one checkpoint,
and a qualitative figure showing how a single prediction evolves across the ODE integration
from $t{=}0$ to $t{=}1$, not just its start and end points. \texttt{ode.sample} was extended
with an opt-in \texttt{return\_trajectory} flag (default off, verified to leave every existing
caller's behavior unchanged) that returns the state after every integration step instead of
only the final one.

The trajectory figure went through two rounds of redesign based on direct feedback on how
useful the first drafts actually were. The first version paired a per-frame image filmstrip
with a line chart of Dice score over the trajectory; this was replaced with a schematic
plotting the lump centroid's $(W,H)$ position at every $t$ (color-graded by $t$, true $180^\circ$
target marked separately), since the position-over-time view is more directly diagnostic of
\emph{whether the model moves the lump at all} than a Dice-vs-$t$ curve is. The second round
rebalanced panel sizes so the frame-by-frame filmstrip --- the main qualitative evidence ---
dominates the figure, with the original/predicted panels shown small alongside it rather than
each claiming half the figure's width.

\hsec{Summary of concrete changes made this session (Part III)}
\begin{itemize}[nosep]
\item Audited every logging site (training diagnostics, qualitative figure annotations, both
  eval scripts) to remove the legacy voxel-agreement/centroid-error metrics, keeping only the
  six trusted mask IoU/Dice numbers (Section~19).
\item Diagnosed and fixed a machine-wide VTK/PyVista GPU memory leak in the off-screen
  rendering path that was silently corrupting every logged figure; verified the fix predates
  and is independent of this session's other changes (Section~20).
\item Restarted the rollout fine-tune under a fresh wandb run id, launched an independent
  from-scratch rollout-only run, and extended the fine-tune by a further 500 epochs once it
  showed real improvement, running both concurrently once GPU headroom allowed it
  (Section~21).
\item Built \texttt{eval\_trajectory.py}: a full-dataset metrics pass plus a per-example
  trajectory figure showing the ODE integration state at every step, iterated twice on figure
  design based on direct feedback (Section~23).
\end{itemize}

\hsec{Part IV --- Choosing and validating the final checkpoint}

\hsub{24. A further layout fix, and a scare that turned out to be a stale checkpoint}
The trajectory figure went through one more redesign round: nesting the top (original/
predicted) and bottom (filmstrip) rows as two independent grids, each filling its own full
width, removed a leftover unused-column gap the earlier version left next to the predicted
panel (\texttt{tight\_layout} does not compact nested grids correctly; switching to
matplotlib's \texttt{constrained\_layout} fixed it). While iterating on this, a full 176-pair
evaluation of the rollout fine-tune's \texttt{latest.pt} (raw, non-EMA weights, epoch
$\approx$1442) came back with combined Dice $\approx 0.19$ --- markedly worse than the
$\approx 0.45$ this same run's own training-time diagnostic had reported shortly before. This
looked like a real regression at first. It was not: the training-time diagnostic evaluates
\emph{EMA} weights (Section~14), while the ad-hoc full evaluation had been pointed at the
\emph{raw} checkpoint --- and, separately, the raw weights kept changing checkpoint-to-
checkpoint (epoch 1442 $\to$ 1464 across successive re-renders) as the run continued past its
best point. Both effects compounded to produce a misleadingly bad number, not a genuine
collapse in the model's ability.

\hsub{25. Selecting the best checkpoint: raw vs.\ EMA, epoch 1250 vs.\ 1500}
To settle this properly, every checkpoint actually saved by the fine-tune run was scored
under identical conditions (same 50 held-out val pairs, same integration settings):
\begin{table}[h]
\centering
\begin{tabular}{@{}lcccc@{}}
\toprule
Checkpoint & lump Dice & pillar Dice & combined Dice & combined IoU \\
\midrule
epoch 1250 (raw)      & 0.451 & 0.153 & 0.422 & --- \\
\textbf{epoch 1250 (EMA)} & \textbf{0.498} & \textbf{0.268} & \textbf{0.478} & \textbf{0.344} \\
epoch 1500 (raw)      & 0.470 & 0.262 & 0.448 & --- \\
epoch 1500 (EMA)      & 0.381 & 0.120 & 0.354 & --- \\
\bottomrule
\end{tabular}
\caption{Every checkpoint saved by the rollout fine-tune run, scored on the same 50 val pairs.
Epoch 1250's EMA weights are the clear winner on every metric.}
\label{tab:checkpoint-selection}
\end{table}
Two things stand out. First, EMA beats raw weights at epoch 1250 but \emph{loses} to raw
weights at epoch 1500 --- the smoothing that helps mid-run becomes a liability once the
underlying weights have drifted past their best point, since EMA is then averaging good
history against a worse recent trend. Second, and more importantly: \textbf{training for
longer did not help}. The extra 250 epochs (1250 $\to$ 1500) requested to see whether the
fine-tune ``was slowly learning'' instead made every metric worse for the EMA weights, and
only partially recovered it for the raw weights. Epoch 1250 EMA is the best model this run
produced, not the final one.

\hsub{26. Final validated result}
Running the full evaluation script on epoch 1250 (EMA) --- all 176 held-out validation pairs,
plus a matched sample of 176 training pairs to check for overfitting --- gives
Table~\ref{tab:final-eval}.
\begin{table}[h]
\centering
\begin{tabular}{@{}lccc@{}}
\toprule
...
\bottomrule
\end{tabular}
\caption{Final checkpoint, full-dataset evaluation. Validation Dice is not lower than train
Dice (gap $-0.048$), ruling out overfitting as an explanation for the result.}
\label{tab:final-eval}
\end{table}

This is, by a wide margin, the best result this task has produced: combined Dice more than
doubled the $\approx 0.19$ figure that briefly looked like the state of the art
(Section~24), and roughly $6\times$ the very first post-blob-fix result of
$\approx 0.07$ (Section~15). Figure~\ref{fig:final-traj} shows the full $t{=}0 \to 1$
trajectory for two held-out validation pairs: the lump visibly separates from its starting
position early in the integration and settles close to its true $180^\circ$ target, with a
clean, compact predicted shape rather than the diffuse multi-component blob of earlier
attempts (Section~13) or the oversized mass seen at later, overtrained checkpoints
(Section~24).
...
\caption{Full $t{=}0 \to 1$ trajectory for the final checkpoint (epoch 1250, EMA weights) on
two held-out validation pairs. Left: the lump centroid's path through the volume, color-graded
by $t$, against the true $180^\circ$ target (green star). Right: original phantom, every
intermediate integration step, and the final prediction.}
\label{fig:final-traj}
\end{figure}

\hsec{Summary of concrete changes made this session (Part IV)}
\begin{itemize}[nosep]
\item Fixed a residual layout bug in the trajectory figure (unused grid columns leaving a
  blank gap) by giving the top and bottom rows independent nested grids under
  \texttt{constrained\_layout} (Section~24).
\item Diagnosed an apparent Dice regression as an evaluation artifact --- raw vs.\ EMA weights
  and a checkpoint that kept changing between renders --- rather than a genuine model
  collapse (Section~24).
\item Scored every checkpoint the rollout fine-tune run actually saved (raw and EMA, at
  epochs 1250 and 1500) under identical conditions and found the extra 250 epochs of training
  made the result \emph{worse}, not better, on both weight variants (Section~25).
\item Selected epoch 1250 (EMA) as the final model and validated it on the full 176-pair
  validation set plus a matched train sample, confirming combined Dice
  $0.453 \pm 0.151$ with no overfitting (Section~26).
\end{itemize}

\hsec{Final results, across the whole project}
Table~\ref{tab:milestones} tracks mask Dice (the trusted metric throughout, Section~15) across
every major fix to the rotate180 task, end to end. Each row is a genuinely different model,
not a re-measurement of the same one: the metric only exists from Section~15 onward, since
mask overlap wasn't computed before that point (earlier stages are annotated qualitatively,
from direct visual inspection of the decoded output).

\begin{table}[h]
\centering
\small
\begin{tabular}{@{}p{4.6cm}ccp{4.4cm}@{}}
\toprule
Stage & Comb.\ Dice & Lump Dice & Notes \\
\midrule
Velocity MSE only (\S11) & --- & --- & prediction never moves off $x_0$ \\
$+$ cross-entropy endpoint (\S12--13) & --- & --- & moves, but a $\sim$20--60-component blob \\
$+$ Dice loss, decay, EMA (\S14) & 0.13 & --- & blob fixed; still low overlap (\S15) \\
$+$ rollout-consistency fine-tune (\S17, \S26) & \textbf{0.453} & \textbf{0.472} & final model, epoch 1250 EMA, full 176-pair val set \\
\bottomrule
\end{tabular}
\caption{Mask Dice across the project's major modeling stages. The rollout-consistency fix
is responsible for roughly a $3.5\times$ improvement in combined Dice over the previous

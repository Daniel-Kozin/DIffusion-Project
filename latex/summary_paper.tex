...
\section{Part I: Interpolating Between Real Phantoms}

\subsection{Goal and pipeline}
...
average has expected norm only $\sqrt{d/2}\approx 0.71\sqrt d$ --- two independent, essentially
...
therefore excellent — the failure diagnosed below is not a symptom of unreliable inversion.
...
\subsection{Results: interpolation failure modes}
...
\subsection{A proposed fix: partial inversion ($t_{\text{mid}}$)}
...
connected-component labeling to the generated foreground and lump masks; real single-lump
phantoms have exactly one connected component of each, so a value of 1 is the target and larger
values indicate torn, fragmented anatomy.

\subsection{$t_{\text{mid}}$ sweep results and diagnosis}
...
\subsection{Why interpolation was abandoned}
...
\section{Part II: A Self-Supervised 180\textdegree{} Rotation Task}
...
starting from the real $x_0$ --- there is no inversion step, and no noise-space midpoint to be
inaccurate about.
...
agreement with the target is still 91.6\% --- background and insert dominate the volume closely
...
\subsection{Fix 1: a cross-entropy endpoint loss}
...
\subsection{Fix 2: Dice loss, decaying schedule, and EMA}
...
\subsection{A trustworthy metric: mask overlap}
Directly computing intersection-over-union and Dice between the predicted and true binary lump
mask,
\begin{equation}
\mathrm{IoU} = \frac{|A\cap B|}{|A\cup B|}, \qquad \mathrm{Dice} = \frac{2|A\cap B|}{|A|+|B|},
\end{equation}
gave mean IoU $\approx0.07$ and Dice $\approx0.13$ across ten validation pairs after Fix 2 ---
nonzero and therefore meaningful, since the two positions are always $180^\circ$ apart and can
never overlap by chance, but far from good. A small blob that happens to contain one true voxel
can score a near-zero IoU while its centroid looks correct, which is exactly why mask overlap
(not centroid distance) is used as the trusted metric throughout the remainder of this section.
...
30 to 100, both left mean combined IoU essentially unchanged ($0.094\to0.094\to0.090$). This
...
\subsection{Fix 3: rollout-consistency loss}
...
multi-component blob of Fix~1 or the oversized mass seen at later, overtrained checkpoints.
...
Table~\ref{tab:milestones} tracks combined mask Dice across the major modeling stages of Part
II end to end. The rollout-consistency fix is responsible for roughly a $3.5\times$ improvement
in combined Dice over the previous best measured result.

\begin{table}[H]
\centering
\small
\begin{tabular}{@{}p{5.0cm}ccp{4.6cm}@{}}
\toprule
Stage & Comb.\ Dice & Lump Dice & Notes \\
\midrule
Velocity MSE only & --- & --- & prediction never moves off $x_0$ \\
$+$ cross-entropy endpoint & --- & --- & moves, but a $\sim$20--60-component blob \\
$+$ Dice loss, decay, EMA & 0.13 & --- & blob fixed; still low overlap \\
$+$ rollout-consistency fine-tune & \textbf{0.453} & \textbf{0.472} & final model, epoch 1250 EMA, full 176-pair validation \\
\bottomrule
\end{tabular}
\caption{Combined mask Dice across the project's major modeling stages on the rotation task.}
\label{tab:milestones}
\end{table}

Two lessons carried across both parts of this project. First, flow matching's lack of a
compression bottleneck is a genuine trade-off, not a free lunch: it is exactly what gives
individual samples and individual-phantom inversion their sharpness, and exactly why the space
it learns is never pushed to be smoothly organized as a whole, which is what real
phantom-to-phantom interpolation would have needed. Second, metrics that ignore the underlying
class imbalance are actively misleading on this data: a model that changes nothing can still
score 91.6\% voxel agreement or a deceptively small centroid error, because the classes that
matter make up under one percent of every volume; mask overlap on those specific classes was the
metric that actually tracked whether the model had learned anything. Once the evaluation caught
up to this and the training loss was made to penalize the model's own multi-step behavior
directly, the rotation task went from a model that never moves its prediction at all to one that
reliably relocates the lump to its true target, with a validated combined Dice of $0.453 \pm
0.151$ and no indication of overfitting to the training set.

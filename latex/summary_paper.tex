...
exact, closed-form target, removing the need to judge success by eye alone; and
...
\end{equation}
gave mean IoU $\approx0.07$ and Dice $\approx0.13$ across ten validation pairs after adding the
Dice term, decay schedule, and EMA,
...
\section{Discussion}
Table~\ref{tab:milestones} tracks combined mask Dice across the rotation task's major modeling
stages end to end. The rollout-consistency fix is responsible for roughly a $3.5\times$
improvement in combined Dice over the previous best measured result.
...
metric that actually tracked whether the model had learned anything.

\paragraph{Limitations.} The dataset is small (40 case IDs, expanded to 984 volumes only through
rotation augmentation), so both the interpolation failure and the rotation-task results should
be read as findings on this specific phantom dataset rather than as general claims about flow
matching at larger scale. The rotation task also sidesteps rather than repairs the original
interpolation goal: it demonstrates that the same velocity field can be trained to hit an exact
target reliably, but it does not, on its own, establish that arbitrary real-to-real interpolation
would work if the same fixes (rollout consistency in particular) were applied back to that
setting.

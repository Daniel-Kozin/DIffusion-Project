...
accuracy -- voxel-level class agreement, lump centroid error -- not just "does it look
plausible". No ODE inversion is used: x0 is already real data, so evaluation is a single
forward integration per pair.
...
def main():
    ...
    table = wandb.Table(columns=["checkpoint", "epoch", "voxel_agreement_mean", "centroid_error_voxels_mean",
    ...
    for ckpt_path in args.checkpoints:
        ...
        print(f"  voxel_agreement={voxel_agreement_mean:.4f}  centroid_error_voxels={centroid_error_mean:.2f}  "
        ...
        table.add_data(ckpt_name, epoch_trained, voxel_agreement_mean, centroid_error_mean,
                        frac_no_lump, pred_components_mean, expected_components_mean)
        summary_rows.append((ckpt_name, epoch_trained, voxel_agreement_mean))

    wandb.log({"eval/summary_table": table})

    print("-" * 60)
    print("Ranked by voxel agreement (higher is better):")
    for ckpt_name, epoch_trained, voxel_agreement_mean in sorted(summary_rows, key=lambda r: -r[2]):
        print(f"  {ckpt_name} (epoch {epoch_trained}): voxel_agreement={voxel_agreement_mean:.4f}")
    print("-" * 60)
